...
function renderFavoriteStations() {
  ...
  function renderStations(items) {
    ...
    items.forEach((station) => {
      ...
      row.dataset.stationUuid = station.stationuuid || "";
  ...
  function selectStation(station, autoplay) {
    ...
    document.querySelectorAll(".facile-radio-row.is-active").forEach((row) => {
      row.classList.remove("is-active");
      row.setAttribute("aria-pressed", "false");
    });
    if (station.stationuuid) {
      const activeRow = document.querySelector('.facile-radio-row[data-station-uuid="' + CSS.escape(station.stationuuid) + '"]');
      if (activeRow) {
